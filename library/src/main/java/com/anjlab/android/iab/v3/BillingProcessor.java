...
public class BillingProcessor extends BillingBase {
    ...
    private boolean purchase(Activity activity, List<String> oldProductIds, String productId,
        ...
        try {
            ...
            if (oldProductIds != null && purchaseType.equals(Constants.PRODUCT_TYPE_SUBSCRIPTION))
                bundle = billingService.getBuyIntentToReplaceSkus(Constants.GOOGLE_API_SUBSCRIPTION_CHANGE_VERSION, contextPackageName, oldProductIds, productId, purchaseType, developerPayload);
            else
                bundle = billingService.getBuyIntent(Constants.GOOGLE_API_VERSION, contextPackageName, productId, purchaseType, developerPayload);
    ...
    public boolean consumePurchase(String productId) {
        if (!isInitialized())
            return false;
        try {
            PurchaseInfo purchaseInfo = getPurchaseTransactionDetails(productId, cachedProducts);
            if (purchaseInfo != null && !TextUtils.isEmpty(purchaseInfo.purchaseData.purchaseToken)) {
                int response = billingService.consumePurchase(Constants.GOOGLE_API_VERSION,
                        contextPackageName, purchaseInfo.purchaseData.purchaseToken);
                if (response == Constants.BILLING_RESPONSE_RESULT_OK) {
